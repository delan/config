# based on <https://github.com/NixOS/nixpkgs/blob/25e53aa156d47bad5082ff7618f5feb1f5e02d01/nixos/modules/services/networking/nsd.nix>
{
  config,
  pkgs,
  lib,
...
let
  cfg = config.services.falling-sky.nsd;  # config.services.nsd;

  username = "fsky-nsd";  # "nsd";
  stateDir = "/var/lib/nsd.falling-sky";  # "/var/lib/nsd";
...
{
  # options are ordered alphanumerically
  options.services.falling-sky.nsd = {  # options.services.nsd = {
  ...
  config = mkIf cfg.enable {
    ...
    environment = {
      ...
      # no file in etc!  # etc."nsd/nsd.conf".source = "${configFile}/nsd.conf";
    };

    users.groups.${username} = {};  # users.groups.${username}.gid = config.ids.gids.nsd;

    users.users.${username} = {
      description = "NSD service user";
      home = stateDir;
      createHome = true;
      isSystemUser = true;  # uid = config.ids.uids.nsd;
      group = username;
    };

    systemd.services.fsky-nsd = {  # systemd.services.nsd = {
